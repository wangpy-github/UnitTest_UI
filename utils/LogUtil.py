...
logdir_path = os.path.join(BASE_DIR, "log")                             # log目录路径
current_time = datetime.datetime.now().strftime("%Y-%m-%d")             # 当前年月日
logfile = os.path.join(logdir_path, current_time + ".log")              # 拼接log文件的路径


# 对外方法
def my_log(log_name=__file__):
    return Logger(log_name=log_name, log_file=logfile, log_level="debug").logger


if __name__ == '__main__':
    my_log().debug("hello word")
